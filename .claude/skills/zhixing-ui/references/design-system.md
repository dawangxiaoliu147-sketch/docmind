...
A 面的"高级感"来自**场景 + 光影层次 + 纪律**，不是来自"素"。五层叠加：

1. **环境顶光** —— 永不用纯平面背景。顶部一抹极低透明度的主色径向光，压在向深色过渡的线性渐变上。
2. **玻璃内沿高光** —— 每个面板带一条内顶高光，这是"玻璃边"的关键：
   ```css
   box-shadow: 0 1px 0 rgba(255,255,255,.06) inset, 0 12px 28px -20px rgba(0,0,0,.5);
   ```
3. **发丝描边纪律** —— 边框 `rgba(255,255,255,.10–.18)` 且**永远 1px**。
4. **主色自发光** —— 主色填充的东西要在自己颜色里发光：
   ```css
   box-shadow: 0 1px 0 rgba(255,255,255,.25) inset,
               0 6px 20px -8px color-mix(in srgb, var(--theme-primary) 55%, transparent);
   ```
5. **胶片噪点** —— 一层固定全屏噪点，`opacity ≈ .03`。

**背板与暗过渡**
- 用**内联 SVG 画真实场景**（层叠山脊 + 雾光 + 微粒），**不要热链外部图片** ——
  跨域图会被 Chromium 的 ORB 静默拦截，首屏直接变灰白；也不要只放一个渐变球（那是 AI 插画）。
- 背板 `position: fixed` 铺满视口，内容盖在它上面（内容 `z-index: 3`，噪点 `z-index: 80`）。
- 图与页面**只通过暗过渡相遇**：一层固定 veil。**两端不要写死纯色**（会把整页压成黑布），
  用 `color-mix(in srgb, var(--theme-bg) 70%, transparent)` 收口、中段 28–30%。
- **没有真实场景时不要叠 veil**。

> ⚠️ **玻璃 ≠ AI 味。** 决定"高级"还是"AI 味"的不是有没有玻璃，而是下面这些纪律：
> 场景色**统一克制**（深绿+柠檬绿 ✅ ／ 亮紫蓝满地 ❌）· 主色**只做高亮和主按钮** ·
> **禁彩色 emoji** · 背板是**真实场景**不是渐变球 · 卡片有**具体内容**不是空壳。
> 把玻璃全删掉换成发丝线，只会得到一张"素"的页面 —— 那不是高级，是没做完。
...
```
.zx-landing
 ├─ .zx-backdrop   内联 SVG 场景：层叠山脊 + 雾光 + 微粒（fixed, z-0）
 ├─ .zx-veil       暗过渡（fixed, z-1，两端用 70% 收口，中段 28–30%）
 ├─ .zx-grain      胶片噪点（fixed, z-80，≈.03）
 └─ .zx-shell      内容（z-3）
     ├─ .zx-nav            玻璃吸顶条（64px）
     ├─ .zx-hero           左：眉标 + H1（主色强调词）+ lead + 双 CTA
     │                     右：.zx-panel 玻璃面板 + 聊天气泡 + 引用片段
     ├─ .zx-metrics        玻璃指标条，主色自发光数字（1px 间隙画发丝分隔）
     ├─ .zx-feats          功能卡 3×N：图标瓦片 + 标题 + 描述 + 徽章 + 箭头
     ├─ .zx-screens        设备格子：3 台统一手机壳，各装一个不同视图（§10）
     ├─ .zx-steps          三步玻璃卡
     ├─ .zx-chips          技术栈等宽 chips
     ├─ .zx-cta            玻璃块 + 环境顶光 + 双按钮
     └─ .zx-footer         玻璃页脚
```
...
- **主色只用在**：眉标、H1 里的强调词、主按钮、图标瓦片、徽章、指标数字、屏内主色。
  **不要**把标题、正文整段染成主色 —— 那才是 AI 味。
- 功能卡是 `.zx-feat`：**图标瓦片 + 标题 + 描述 + 底部徽章 + 箭头**，hover 上浮 3px + 主色描边。
  图标用**单调 Unicode**（`◈ ✦ ≋ ⊞ ⬢ ◐ ⊹ ⊡ ↯`），**绝不用彩色 emoji**。
- 徽章 = **主色实心底 + 8px 圆角 + 等宽大写**，不要空心描边胶囊。
- **界面一览（§10 设备格子）**：3 台**完全相同的手机壳**，只换屏内内容；
  每台底栏高亮当前 tab；说明文字用等宽小字 `<b>名字</b> · 一句功能`。
  手机壳宽度不足时**堆成列**，不许缩小壳去塞更多台。
- 收尾 CTA 是**最后一个让文字坐在背板上的位置**，配一块玻璃 + 环境顶光。
...
### Z8 · 把"去 AI 味"理解成"变素" = 过度矫正
**现象**：第一版是玻璃堆叠 + 亮紫蓝满地，用户说"AI 味太重"；于是我删掉玻璃、改成发丝线排版、
主色只留给主按钮 —— 用户立刻回："**太素了**，我要那种效果"。
**错误**：把"去 AI 味"等同于"去掉视觉层次"。结果页面没有场景、没有光影、没有质感，
只剩灰底和细线 —— **那不是高级，是没做完**。
**正确**：AI 味的来源**不是**玻璃或卡片，而是这五条：
1. 底色是**带蓝紫的近黑** + 强调色是**亮紫蓝**，还**满屏都是**
2. 主色染到了**标题、数字、图标、装饰线**上
3. 用**彩色 emoji** 当图标
4. 背板是**随机渐变球**，不是真实场景
5. 卡片是**空壳**（只有图标 + 徽章 + 箭头，没有具体内容）

**修这五条，而不是删玻璃。** 玻璃面板、场景背板、主色自发光、设备陈列**全部保留**。
**同步位置**：§2.2 / §3.1 / §8 / §12

### Z9 · 场景色决定"高级"还是"廉价"
**现象**：同样是深色玻璃 + 主色自发光，`#9aa6ff`（亮紫蓝）显得廉价，`#d7ef83`（柠檬绿）显得高级。
**正确**：选**辨识度高、饱和度受控、不与深色底顺色**的强调色。三套预设
（`rain` 深绿+柠檬绿 / `snow` 冷蓝白 / `cloud` 暖灰夜色）都比"AI 默认紫蓝"高级。
换场景只改 16 个 `--theme-*` 令牌，组件一行都不用动。
**同步位置**：§2.2
...
> ⚠️ **先看这条，别过度矫正**：去 AI 味 **≠ 变素**。
> 玻璃面板、场景背板、主色自发光、光影层次、设备陈列 —— **全都要保留**。
> 要砍的只是下面这些**具体的坏味道**。把整页做成灰底细线 = 没做完，不是高级（见 Z8）。
