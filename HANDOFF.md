...
- 最后核实：2026-09-20（本次只做**只读核查**，未改任何代码）
...
- 已知【已核实】：`97255a9` 已在 `origin/main`（本地/远端 `0 0` 差异）；`chat-shell.tsx` 的 `newConvId()` 写法正确。
- 未知【未核实】：线上跑的那个 bundle 里是不是这一份。
- **第一动作**：用**隐身窗口**打开 `http://39.105.158.60:3000` → 登录 → 知识库聊天页 → 点「新建对话」。
  - 修好的样子：右侧从「点击「新建对话」开始提问…」变成输入框，地址栏出现 `?conv=<uuid>`。
  - 仍无反应：按 F12，把 Console 里红色报错的**原文**贴回来。不要再猜。
  - **为什么必须隐身窗口**：旧 JS chunk 很可能还在缓存里，普通窗口刷新可能仍执行**没修的那份 bundle** → 得到"修复失败"的**假阴性**。普通窗口请用 Ctrl+Shift+R。
- 拿到结果前**不要动代码**（包括不要"顺手加固"）。
...
- **仍缺【未核实】**：线上隐身窗口实测。
- **完成判定**：隐身窗口点击后右侧出现输入框 + 地址栏 `?conv=<uuid>`。
- **若仍不行**：看 Console 报错。若**无任何报错**，说明不在 JS 异常层，改查 `router.push` 同路由 searchParams 更新、或 `(app)/layout.tsx` 容器。
